...
(deftest test-get-service-catalog
  (with-redefs [http/post (fn [url data] payload/login)]
    (let [response (identity/login "alice" :apikey "0123456789abcdef")
          data (identity/get-service-catalog response)]
      (is (= 10 (count data))))))
...
(deftest test-get-cloud-servers-endpoints
  (with-redefs [http/post (fn [url data] payload/login)]
    ; get default version
    (let [response (identity/login "alice" :apikey "0123456789abcdef")
          data (identity/get-cloud-servers-endpoints response)]
      (is (= 4 (count data))))
    ; get version 2 explicitly
    (let [response (identity/login "alice" :apikey "0123456789abcdef")
          data (identity/get-cloud-servers-endpoints response :version 2)]
      (is (= 4 (count data))))
    ; get version 1
    (let [response (identity/login "alice" :apikey "0123456789abcdef")
          data (identity/get-cloud-servers-endpoints response :version 1)]
      (is (= 1 (count data))))))

(deftest test-list-cloud-servers-regions
  (with-redefs [http/post (fn [url data] payload/login)]
    (let [response (identity/login "alice" :apikey "0123456789abcdef")
          data (identity/list-cloud-servers-regions response)]
      (is (= [:dfw :iad :ord :syd] (sort data))))))

(deftest test-get-cloud-servers-region
  (with-redefs [http/post (fn [url data] payload/login)]
    (let [response (identity/login "alice" :apikey "0123456789abcdef")
          data (identity/get-cloud-servers-region response :dfw)]
...
(deftest test-get-cloud-servers-region-url
  (with-redefs [http/post (fn [url data] payload/login)]
    (let [response (identity/login "alice" :apikey "0123456789abcdef")
          data (identity/get-cloud-servers-region-url response :ord)]
